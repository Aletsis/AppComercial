...
public class CreateProductoCommandHandler : IRequestHandler<CreateProductoCommand, int>
{
    ...
    public CreateProductoCommandHandler(IContpaqiSdk sdk, IContpaqiDbContext context)
    {
        _sdk = sdk;
        _context = context;
    }
    ...
    {
        // Validar si el código ya existe para evitar error del SDK
        var existe = await _context.Productos.AnyAsync(p => p.CCODIGOPRODUCTO == request.Codigo, cancellationToken);
        if (existe)
            throw new Exception($"El código de producto '{request.Codigo}' ya existe en CONTPAQi.");
